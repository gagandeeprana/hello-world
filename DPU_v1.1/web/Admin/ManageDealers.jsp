<%@page import="dpu.services.admin.DealersServicesImpl"%>
<%@page import="dpu.services.admin.DealersServicesDAO"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="dpu.beans.admin.DealersBean"%>
<%@page import="java.util.List"%>
<!DOCTYPE html>
<!--
To change this license header, choose License Headers in Project Properties.
To change this template file, choose Tools | Templates
and open the template in the editor.
-->
<html>
    <head>
        <jsp:include page="stylesheet.jsp"/>
        <title>TODO supply a title</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width">
    </head>
    <body>
        <jsp:include page="mp_header.jsp"/>
        <% DealersServicesDAO objServices = new DealersServicesImpl();
            List<DealersBean> lstDealers = null;
            if (request.getParameter("sbtnSearch") != null) {
                String Location = request.getParameter("txtLocation");
                String search = request.getParameter("txtSearch");
                lstDealers = objServices.getAllDealers(Location, search);
            } else {
                lstDealers = objServices.getAllDealers("", "");
            }
            pageContext.setAttribute("LIST_DEALERS", lstDealers);
        %>
        <form action="#" method="GET">
            <form action="#" method="GET">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-2">

                        </div>
                        <div class="col-md-8 ">
                            <div class="panel panel-success">
                                <div class="panel-body">
                                    <div align="center"><h2><b>Manage Dealers</b></h2></div>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="form-group ">
                                                <c:if test="${sessionScope.userType.equalsIgnoreCase('admin')}">
                                                    <input class="btn btn-success " type="button" name="sbtnSearch" value="Add Dealer" onclick="window.location = 'AddDealer.jsp'">
                                                </c:if>


                                            </div>



                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <select name="ddlStatus" class="form-control">
                                                    <option value="">Status</option>
                                                    <option value="1" ${STATUS_1}>Active</option>
                                                    <option value="0" ${STATUS_0}>InActive</option>
                                                </select>




                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">

                                                <input class="form-control pull-right" placeholder="Name" name="txtSearch" type="text" value="">






                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">



                                                <input class="btn btn-success  " type="submit" name="sbtnSearch" value="Search">




                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-12 col-xs-6 ">
                                            <table class="table table-hover table-responsive ">

                                                <thead>
                                                    <tr>
                                                        <th>
                                                            S.No
                                                        </th>
                                                        <th>
                                                            Name
                                                        </th>
                                                        <th>
                                                            Contact
                                                        </th>
                                                        <th>
                                                            Email
                                                        </th>
                                                        <th>
                                                            Status
                                                        </th>
                                                        <th>
                                                            Links
                                                        </th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <c:forEach items="${LIST_DEALERS}" var="obj" varStatus="i">
                                                        <tr>
                                                            <td>${i.index+1}</td>
                                                            <td>${obj.name}</td>
                                                            <td>${obj.contact}</td>
                                                            <td>${obj.mail}</td>
                                                            <td>
                                                                <c:choose>
                                                                    <c:when test="${obj.status==0}">
                                                                        <c:set var="status" value="InActive"></c:set>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <c:set var="status" value="Active"></c:set>
                                                                    </c:otherwise>
                                                                </c:choose>

                                                                <a href="ControllerDealer.jsp?dealerId=${obj.dealerId}&status=${obj.status}">${status}</a></td>
                                                            <td><a href="EditDealer.jsp?dealerId=${obj.dealerId}">View & Edit</a></td>
                                                        </tr>
                                                    </c:forEach>  
                                                    <%--<c:forEach items="${LIST_DEALERS}" var="obj" varStatus="i">--%>
                                                    <!--                                <tr>
                                                                                        <td>${i.index+1}</td>
                                                                                        <td>${obj.name}</td>
                                                                                        <td>${obj.contact}</td>
                                                                                        <td>${obj.mail}</td>
                                                                                        <td>
                                                    <c:choose>
                                                        <c:when test="${obj.status==0}">
                                                            <c:set var="status" value="InActive"></c:set>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <c:set var="status" value="Active"></c:set>
                                                        </c:otherwise>
                                                    </c:choose>
            
                                                    <a href="ControllerDealer.jsp?dealerId=${obj.dealerId}&status=${obj.status}">${status}</a></td>
                                                <td><a href="EditDealer.jsp?dealerId=${obj.dealerId}">View & Edit</a></td>
                                            </tr>-->
                                                    <%--</c:forEach>--%>  
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-2">

                        </div>
                    </div>
                </div>

















                <!--            <table align="center" cellpadding="5">
                                <tr>
                                    <td align="center">
                                        <strong>
                                            Manage Dealers
                                        </strong>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <label id="lblMsg" style="color:red">
                                            <small>${param.msg}</small>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="100%">
                                        <table width="100%">
                                            <tr>
                                                <td>
                                                    <input type="button" onclick="window.location = 'AddDealer.jsp'" class="button" name="sbtnAddDealer" value="Add New Dealer"/>
                                                </td>
                                                <td>
                                                    <label><b>Location</b></label>
                                                </td>
                                                <td>
                                                    <input type="text" class="txtStyle" name="txtLocation"/>
                                                </td>
                                                <td>
                                                    <label><b>Name</b></label>
                                                </td>
                                                <td>
                                                    <input type="text" class="txtStyle" name="txtSearch" />
                                                </td>
                                                <td>
                                                    <input type="submit" class="button" name="sbtnSearch" value="Search"/>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <table style="border-collapse: collapse; border-style: inset" align = "center" width="100%" border="1">
                                            <tr>
                                                <th>
                                                    S.No
                                                </th>
                                                <th>
                                                    Name
                                                </th>
                                                <th>
                                                    Contact
                                                </th>
                                                <th>
                                                    Email
                                                </th>
                                                <th>
                                                    Status
                                                </th>
                                                <th>
                                                    Links
                                                </th>
                                            </tr>
                <c:forEach items="${LIST_DEALERS}" var="obj" varStatus="i">
                    <tr>
                        <td>${i.index+1}</td>
                        <td>${obj.name}</td>
                        <td>${obj.contact}</td>
                        <td>${obj.mail}</td>
                        <td>
                    <c:choose>
                        <c:when test="${obj.status==0}">
                            <c:set var="status" value="InActive"></c:set>
                        </c:when>
                        <c:otherwise>
                            <c:set var="status" value="Active"></c:set>
                        </c:otherwise>
                    </c:choose>

                    <a href="ControllerDealer.jsp?dealerId=${obj.dealerId}&status=${obj.status}">${status}</a></td>
                <td><a href="EditDealer.jsp?dealerId=${obj.dealerId}">View & Edit</a></td>
            </tr>
                </c:forEach>         
            </table>
        </td>
    </tr>
</table>-->
            </form>
            <jsp:include page="mp_footer.jsp"/>
    </body>
</html>
