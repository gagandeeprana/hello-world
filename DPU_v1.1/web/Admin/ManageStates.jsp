<%@page import="dpu.services.admin.StateServicesImpl"%>
<%@page import="dpu.services.admin.StateServicesDAO"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@page import="dpu.beans.admin.StateBean"%>
<%@page import="java.util.List"%>
<!DOCTYPE html>
<!--
To change this license header, choose License Headers in Project Properties.
To change this template file, choose Tools | Templates
and open the template in the editor.
-->
<html>
    <head>
        <jsp:include page="stylesheet.jsp"/>
        <title>TODO supply a title</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width">
    </head>
    <body>
        <jsp:include page="mp_header.jsp"/>
        <%
            StateServicesDAO objServices = new StateServicesImpl();
            int cId = 0;
            String cName = "";
            List<StateBean> lstStates = null;
            if (request.getParameter("countryId") != null) {
                cId = Integer.parseInt(request.getParameter("countryId"));
                cName = request.getParameter("countryName");
                session.setAttribute("cId", cId);
                session.setAttribute("cName", cName);
            } else {
                cId = Integer.parseInt(session.getAttribute("cid").toString());
                cName = session.getAttribute("cName").toString();
            }
            if (request.getParameter("sbtnSearch") != null) {
                lstStates = objServices.getAllStates(cId, request.getParameter("txtSearch"));
            } else {
                lstStates = objServices.getAllStates(cId, "");
            }
            pageContext.setAttribute("LIST_STATES", lstStates);

        %>
        <form>

            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-2">

                    </div>
                    <div class="col-md-8 ">
                        <div class="panel">
                            <div class="panel-body">
                                    <div align="center"><h2><b>Manage States</b></h2></div>
                                <div class="row" style="margin-bottom: 5px;">
                                    <div class="col-md-9 ">

                                    </div>
                                    <div class="col-md-3 ">
                                        <b>Country: </b>${sessionScope.cName}
                                    </div>

                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group ">
                                            <input class="btn btn-success " type="button" name="sbtnSearch" value="<" onclick="window.location = 'ManageCountry.jsp'">
                                            <input class="btn btn-success " type="button" name="sbtnSearch" value="Add State" onclick="window.location = 'AddState.jsp'">


                                        </div>



                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">

                                            <input class="form-control pull-right" placeholder="StateName" name="txtSearch" type="text" value="">






                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">



                                            <input class="btn btn-success  " type="submit" name="sbtnSearch" value="Search">




                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12 col-xs-12 ">
                                        <table class="table table-hover table-responsive ">

                                            <thead>
                                                <tr>
                                                    <th>S.no</th>
                                                    <th>State Name</th>
                                                    <th>Status</th>
                                                    <th>Links</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <c:forEach items="${LIST_STATES}" var="obj" varStatus="i">
                                                    <tr>
                                                        <td>${i.index+1}</td>
                                                        <td>${obj.name}</td>
                                                        <td> <c:choose>
                                                                <c:when test="${obj.status==0}">
                                                                    <c:set var="status" value="InActive"></c:set>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <c:set var="status" value="Active"></c:set>
                                                                </c:otherwise>
                                                            </c:choose>
                                                            <a href="ControllerState.jsp?stateId=${obj.stateId}&status=${obj.status}">${status}</a></td>
                                                        <td>
                                                            <a href="EditState.jsp?stateId=${obj.stateId}">View & Edit</a>&nbsp;/&nbsp;
                                                            <a href="ManageCities.jsp?stateId=${obj.stateId}&stateName=${obj.name}">Cities</a>
                                                        </td>
                                                    </tr>
                                                </c:forEach>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-2">

                            </div>
                        </div>
                    </div>
                </div>
            </div>


















            <!--            
                        <table align="center" cellpadding="5">
            
                            <tr>
                                <td align="center">
                                    <strong>
                                        Manage States
                                    </strong>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label id="lblMsg" style="color:red">
                                        <small>${param.msg}</small>
                                    </label>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <b>Country :</b>
                                    <label>${sessionScope.cName}</label>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <table width="100%">
                                        <tr>
                                            <td>
                                                <input type="button" onclick="window.location = 'AddState.jsp'" class="button" value="Add State"/>
                                            </td>
                                            <td align="right">
                                                <input type="text" class="txtStyle" name="txtSearch"/>
                                                <input type="submit" class="button" name="sbtnSearch" value="Search"/>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <table width="100%" style="border-collapse: collapse; border-style: inset" border="1">
                                        <tr>
                                            <th>
                                                S.No.
                                            </th>
                                            <th>
                                                State Name
                                            </th>
                                            <th>
                                                Status
                                            </th>
                                            <th>
                                                Links
                                            </th>
                                        </tr>
            <c:forEach items="${LIST_STATES}" var="obj" varStatus="i">
                <tr>
                    <td>${i.index+1}</td>
                    <td>${obj.name}</td>
                    <td> <c:choose>
                    <c:when test="${obj.status==0}">
                        <c:set var="status" value="InActive"></c:set>
                    </c:when>
                    <c:otherwise>
                        <c:set var="status" value="Active"></c:set>
                    </c:otherwise>
                </c:choose>
                <a href="ControllerState.jsp?stateId=${obj.stateId}&status=${obj.status}">${status}</a></td>
            <td>
                <a href="EditState.jsp?stateId=${obj.stateId}">View & Edit</a>&nbsp;/&nbsp;
                <a href="ManageCities.jsp?stateId=${obj.stateId}&stateName=${obj.name}">Cities</a>
            </td>
        </tr>
            </c:forEach>
        </table>
    </td>
</tr>
</table>-->
        </form>
        <jsp:include page="mp_footer.jsp"/>
    </body>
</html>
