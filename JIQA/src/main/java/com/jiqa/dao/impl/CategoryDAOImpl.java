...
public class CategoryDAOImpl implements CategoryDAO {
	...
	public int softDeleteCategory(int status, int categoryId) {
		...
		try {
			session = sessionFactory.openSession();
			query = session.createQuery("update categorymaster set status = :status where category_is = :categoryId");
			query.setParameter("status", status);
			query.setParameter("categoryId", categoryId);
			result = query.executeUpdate();
		} catch (Exception e) {
			logger.error("CategoryDAOImpl: Inside softDeleteCategory: Exception is: " + e.getMessage());
		} finally {
			try {
				if (session != null) {
					session.close();
				...
				logger.error("CategoryDAOImpl: Inside softDeleteCategory: Inside Finally: Exception is: " + e2.getMessage());
			}
		}
		return result;
	}
