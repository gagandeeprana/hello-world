...
CREATE TABLE IF NOT EXISTS `dpu`.`straighttruckmaster` (
  `unit_no` INT NOT NULL AUTO_INCREMENT,
  `straight_truck_class` VARCHAR(45) NULL,
  `VIN` VARCHAR(45) NULL,
  `make` VARCHAR(45) NULL,
  `model` VARCHAR(45) NULL,
  `straight_truck_year` INT NULL,
  `plate_no` VARCHAR(45) NULL,
  ...
  CONSTRAINT `tracking_id_straight_truck`
    FOREIGN KEY (`tracking_id`)
    REFERENCES `dpu`.`trackingmaster` (`tracking_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `terminal_id_straight_truck`
    FOREIGN KEY (`terminal_id`)
    REFERENCES `dpu`.`terminalmaster` (`terminal_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `category_id_straight_truck`
...
CREATE TABLE IF NOT EXISTS `dpu`.`drivermaster` (
  ...
  CONSTRAINT `division_id_drivermaster`
    FOREIGN KEY (`division_id`)
    REFERENCES `dpu`.`divisionmaster` (`division_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `terminal_id_drivermaster`
    FOREIGN KEY (`terminal_id`)
    REFERENCES `dpu`.`terminalmaster` (`terminal_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `category_id_drivermaster`
    FOREIGN KEY (`category_id`)
    REFERENCES `dpu`.`categorymaster` (`category_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `class_id_drivermaster`
